...
class Command(BaseCommand):
    help = 'Инициализирует базу данных и заполняет ее фейковыми авторами и постами'
    ...
    def handle(self, *args, **options):
        ...
        for i in range(count):
            author = Author(author_name=f'Author_{i}', author_email=f'mail{i}@mail.ru')
            author.save()
            for j in range(count):
                Post(
                    post_title=f'Title_{j}',
                    post_content=" ".join(choices(text, k=64)),
                    post_author=author
                ).save()
